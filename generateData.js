...
for (var i = 1; i <= 336; i++) {
  database.cats.push({
    ...
    image: encodeURI(`http://${config.get('host')}:${config.get('port')}/images/cat/cat-unsplash-${i}.jpg`),
...
recipes.forEach((item) => {
  ...
  let imgPath = encodeURI(`http://${config.get('host')}:${config.get('port')}/images/meals/${filename}`);
  let tagsArray = [];
